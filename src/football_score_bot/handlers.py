...
MENU_REFERRALS_TEXTS = {"👥 推广", "👥 推广邀请", "推广", "推广邀请", "邀请返佣", "👥 Referral", "Referral"}
...
def build_router(
    ...
    async def wallet_ledger(callback: CallbackQuery) -> None:
        ...
        lang = await _callback_lang(callback, database, settings)
        await callback.message.answer(_format_ledger(rows, lang), reply_markup=_wallet_keyboard(lang))
    ...
    async def withdraw_confirm(callback: CallbackQuery, state: FSMContext) -> None:
        ...
        lang = await _callback_lang(callback, database, settings)
        await callback.message.answer(
            f"Withdrawal request submitted. A 10% fee will apply. Please wait for admin review.\nRequest ID: {request['id']}"
            if lang == "en"
            else f"提现申请已提交，手续费 10%，请等待管理员审核。\n申请号：{request['id']}",
    ...
    async def withdraw_request(message: Message, command: CommandObject) -> None:
        ...
        if not request:
            ...
            return
        await message.answer(f"提现申请已提交，手续费 10%，请等待管理员审核。\n申请号：{request['id']}")
    ...
    async def show_referrals_page(message: Message, user_id: int, lang: str) -> None:
        code = await database.get_referral_code(user_id)
        ...
        application = await database.get_latest_agent_application(user_id)
        await message.answer(
    ...
    async def open_referrals_from_message(message: Message, source: str) -> None:
        lang = await _remember_user_and_lang(message, database, settings)
        if not message.from_user:
            return
        logger.info(
            "referrals_opened source=%s user_id=%s lang=%s",
            source,
            message.from_user.id,
            lang,
        )
        await show_referrals_page(message, message.from_user.id, lang)

    @router.message(Command("referrals"))
    async def referrals_cmd(message: Message) -> None:
        await open_referrals_from_message(message, "command")

    @router.message(F.text.in_(_translated_texts("referrals") | MENU_REFERRALS_TEXTS))
    async def referrals_menu(message: Message) -> None:
        await open_referrals_from_message(message, "menu")

    @router.callback_query(F.data.in_({"referrals", "referral", "invite", "menu:referrals", "nav:referrals"}))
    async def referrals_cb(callback: CallbackQuery) -> None:
        await _safe_callback_answer(callback)
        if not callback.message or not callback.from_user:
            return
        lang = await _callback_lang(callback, database, settings)
        logger.info(
            "referrals_opened source=%s user_id=%s lang=%s callback_data=%s",
            "callback",
            callback.from_user.id,
            lang,
            callback.data,
        )
        await show_referrals_page(callback.message, callback.from_user.id, lang)
    ...
    async def rebate_apply(callback: CallbackQuery, state: FSMContext) -> None:
        ...
        await state.clear()
        lang = await _callback_lang(callback, database, settings)
        snapshot = await database.get_agent_rebate_snapshot(callback.from_user.id)
        claimable_stake = Decimal(str(snapshot.get("claimable_stake") or snapshot.get("available_rebate_stake") or 0))
        await state.update_data(
            note="agent rebate request",
            turnover=str(claimable_stake),
            rebate_rate=str(snapshot.get("rebate_rate") or "0"),
            rebate_amount=str(snapshot.get("claimable_rebate") or snapshot.get("estimated_rebate_amount") or "0"),
        )
        if claimable_stake <= 0:
            await callback.message.answer(
                _format_agent_rebate_unavailable(snapshot, settings.wallet_currency, lang),
                reply_markup=_agent_rebate_keyboard(False, lang),
            )
            return
        await state.set_state(RebateStates.confirming_request)
        await callback.message.answer(
            _format_rebate_confirm(await state.get_data(), settings.wallet_currency, lang),
            reply_markup=_rebate_confirm_keyboard(lang),
        )
    ...
    async def admin_panel_callback(callback: CallbackQuery) -> None:
        ...
        if data == "admin:stats":
            ...
            claimable_stake = Decimal(str(snapshot.get("claimable_stake") or snapshot.get("available_rebate_stake") or 0))
            await callback.message.answer(
                _format_agent_rebate_page(snapshot, settings.wallet_currency, lang),
                reply_markup=_agent_rebate_keyboard(claimable_stake > 0, lang),
            )
...
def _format_admin_referral_summary(summary: dict, currency: str) -> str:
    return (
        ...
        f"有效下级：{summary.get('active_count') or 0} 人\n"
        f"累计下级充值：{_money(summary.get('total_deposit'))} {currency}\n"
        f"累计流水：{_money(summary.get('total_turnover'))} {currency}\n"
        f"待结算返佣：{_money(summary.get('pending_commission'))} {currency}\n"
        f"已结算返佣：{_money(summary.get('settled_commission'))} {currency}\n"
        f"待结算返水：{_money(summary.get('pending_rebate'))} {currency}"
    )
...
def _referral_keyboard(role: str = "user", application: dict | None = None, lang: str = "zh") -> InlineKeyboardMarkup:
    if lang == "en":
        ...
        if role in {"agent", "admin", "super_admin"}:
            rows.append([InlineKeyboardButton(text="Subordinates", callback_data="referrals:children")])
            rows.append([InlineKeyboardButton(text="Reward Records", callback_data="referrals:commissions")])
        rows.extend(
            [
    ...
    if role in {"agent", "admin", "super_admin"}:
            rows.append([InlineKeyboardButton(text="下级管理", callback_data="referrals:children")])
    else:
        status = str((application or {}).get("status") or "")
        if status == "pending":
            rows.append([InlineKeyboardButton(text="代理申请审核中", callback_data="referrals:agent_pending")])
...
# Final UI overrides for agent rebate, ledger, referral, bets, and withdraw copy.
...
def _format_bets_page(rows: list[dict], stats: dict, status_group: str, settings: Settings, lang: str = "zh") -> str:
    ...
    settled_count = int(stats.get("settled_count") or 0)
    if lang == "en":
        lines = ["📊 My Bets", "", f"Pending: {pending_count + manual_required_count}", f"Settled: {settled_count}", ""]
        empty = "No bets yet."
    else:
        lines = ["📊 我的注单", "", f"待开奖：{pending_count + manual_required_count} 张", f"已开奖：{settled_count} 张", ""]
        empty = "暂无注单。"
    if not rows:
        lines.append(empty)
        return "\n".join(lines)
    for index, bet in enumerate(rows, 1):
        status = _bet_status_label(str(bet.get("status")), lang)
        lines.append(
            f"{index}. {bet.get('bet_no') or bet.get('id')} | {bet.get('fixture_label') or '-'} | "
            f"{_money(bet.get('stake'))} {settings.wallet_currency} | {status}"
        )
    return "\n".join(lines)


def _rebate_confirm_keyboard(lang: str = "zh") -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup(
        inline_keyboard=[
            [InlineKeyboardButton(text="Submit Request" if lang == "en" else "提交申请", callback_data="rebate:confirm")],
            [InlineKeyboardButton(text="Cancel" if lang == "en" else "取消", callback_data="fsm_cancel:referrals")],
        ]
    )


def _agent_rebate_keyboard(can_submit: bool, lang: str = "zh") -> InlineKeyboardMarkup:
    rows = []
    if can_submit:
        rows.append([InlineKeyboardButton(text="Submit Rebate Request" if lang == "en" else "提交返水申请", callback_data="referrals:rebate_apply")])
    rows.extend(
        [
            [InlineKeyboardButton(text="Rebate Records" if lang == "en" else "返水申请记录", callback_data="referrals:rebates")],
            [InlineKeyboardButton(text="Back to Admin Panel" if lang == "en" else "返回管理员面板", callback_data="admin")],
        ]
    )
    return InlineKeyboardMarkup(inline_keyboard=rows)


def _format_rebate_confirm(data: dict[str, Any], currency: str = "USDT", lang: str = "zh") -> str:
    if lang == "en":
        return "\n".join(
            [
                "🎁 Rebate Request",
                "",
                f"Eligible Stake Amount: {_money(data.get('turnover'))} {currency}",
                f"Rebate Rate: {_percent_text(data.get('rebate_rate'))}",
                f"Estimated Rebate: {_money(data.get('rebate_amount'))} {currency}",
    ...
    return "\n".join(
        [
            "🎁 返水申请",
            "",
            f"本次申请投注额：{_money(data.get('turnover'))} {currency}",
            f"返水比例：{_percent_text(data.get('rebate_rate'))}",
            f"预计返水：{_money(data.get('rebate_amount'))} {currency}",
            "申请说明：默认",
        ]
    )


def _format_agent_rebate_unavailable(snapshot: dict, currency: str, lang: str = "zh") -> str:
    valid_stake = snapshot.get("valid_stake") or snapshot.get("total_valid_stake")
    settled_stake = snapshot.get("settled_rebate_stake")
    if lang == "en":
        return "\n".join(
            [
                "No rebate is currently available.",
                f"Settled valid stake: {_money(valid_stake)} {currency}",
                f"Already claimed/settled rebate stake: {_money(settled_stake)} {currency}",
                f"Currently eligible stake: 0 {currency}",
            ]
        )
    return "\n".join(
        [
            "当前暂无可申请返水资格。",
            f"已结算有效投注：{_money(valid_stake)} {currency}",
            f"已申请/已结算返水基数：{_money(settled_stake)} {currency}",
            f"当前可申请返水投注额：0 {currency}",
        ]
    )


def _format_agent_rebate_page(snapshot: dict, currency: str, lang: str = "zh") -> str:
    valid_stake = snapshot.get("valid_stake") or snapshot.get("total_valid_stake")
    settled_stake = snapshot.get("settled_rebate_stake")
    claimable_stake = snapshot.get("claimable_stake") or snapshot.get("available_rebate_stake")
    rebate_rate = snapshot.get("rebate_rate")
    rebate_amount = snapshot.get("claimable_rebate") or snapshot.get("estimated_rebate_amount")
    if _decimal_value(claimable_stake) <= 0:
        title = "🎁 My Rebate" if lang == "en" else "🎁 我的返水"
        return f"{title}\n\n{_format_agent_rebate_unavailable(snapshot, currency, lang)}"
    if lang == "en":
        return "\n".join(
            [
                "🎁 My Rebate",
                "",
                f"Settled valid stake: {_money(valid_stake)} {currency}",
                f"Already claimed rebate stake: {_money(settled_stake)} {currency}",
                f"Currently eligible stake: {_money(claimable_stake)} {currency}",
                f"Current rebate rate: {_percent_text(rebate_rate)}",
                f"Estimated rebate: {_money(rebate_amount)} {currency}",
            ]
        )
    return "\n".join(
        [
            ...
            "",
            f"已结算有效投注：{_money(valid_stake)} {currency}",
            f"已申请返水基数：{_money(settled_stake)} {currency}",
            f"当前可申请投注额：{_money(claimable_stake)} {currency}",
            f"当前返水比例：{_percent_text(rebate_rate)}",
            f"预计返水：{_money(rebate_amount)} {currency}",
        ]
    )
...
def _withdraw_confirm_keyboard(lang: str = "zh") -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup(
        inline_keyboard=[
            [InlineKeyboardButton(text="Submit Request" if lang == "en" else "提交申请", callback_data="withdraw:confirm")],
            [InlineKeyboardButton(text="Cancel" if lang == "en" else "取消", callback_data="fsm_cancel:wallet")],
        ]
    )
...
def _format_withdraw_confirm(data: dict[str, Any], currency: str, lang: str = "zh") -> str:
    ...
    fee = (amount * Decimal("0.10")).quantize(Decimal("0.01"))
    net = (amount * Decimal("0.90")).quantize(Decimal("0.01"))
    if lang == "en":
        return "\n".join(
            [
                "🏧 Confirm Withdrawal",
                "",
                "Withdrawal fee is 10%. Actual received amount = withdrawal amount * 90%.",
                "",
                f"Withdrawal amount: {_money(amount)} {currency}",
                f"Fee: {_money(fee)} {currency}",
                f"Estimated received: {_money(net)} {currency}",
                f"Network: {_network_label(str(data.get('network') or ''))}",
                f"Address: {data.get('address') or '-'}",
    ...
    return "\n".join(
        [
            "🏧 确认提现申请",
            "",
            "提现手续费为 10%，实际到账金额 = 提现金额 * 90%。",
            "",
            f"申请提现：{_money(amount)} {currency}",
            f"手续费：{_money(fee)} {currency}",
            f"预计到账：{_money(net)} {currency}",
            f"网络：{_network_label(str(data.get('network') or ''))}",
            f"地址：{data.get('address') or '-'}",
...
def _ledger_reason_label(ledger_type: str, lang: str = "zh") -> str:
    en = {
        "bet_freeze": "Bet Stake Freeze",
        "bet_win_payout_frozen": "Win Payout Frozen",
        "bet_win_payout": "Win Payout",
        "payout_unlock": "Frozen Payout Unlock",
        "payout_unfreeze": "Frozen Payout Unlock",
        "bet_win_payout_unlock": "Frozen Payout Unlock",
        "frozen_release": "Frozen Release",
        "agent_rebate": "Agent Rebate",
        "rebate": "Rebate",
        "referral_reward": "Referral Reward",
        "commission": "Referral Reward",
        "manual_adjust": "Manual Adjustment",
        "deposit": "Deposit Paid",
        "deposit_paid": "Deposit Paid",
        "deposit_manual": "Deposit Paid",
        "withdraw_freeze": "Withdrawal Freeze",
        "withdraw_paid": "Withdrawal Paid",
        "withdraw_reject_refund": "Withdrawal Rejected Refund",
        "admin_freeze": "Admin Freeze",
        "admin_unfreeze": "Admin Unfreeze",
    }
    zh = {
        "bet_freeze": "注单冻结",
        "bet_win_payout_frozen": "中奖派彩冻结",
        "bet_win_payout": "中奖派彩",
        "payout_unlock": "冻结派奖解冻",
        "payout_unfreeze": "冻结派奖解冻",
        "bet_win_payout_unlock": "冻结派奖解冻",
        "frozen_release": "冻结资金释放",
        "agent_rebate": "返水入账",
        "rebate": "返水入账",
        "referral_reward": "返佣入账",
        "commission": "返佣入账",
        "manual_adjust": "手工调账",
        "deposit": "充值入账",
        "deposit_paid": "充值入账",
    ...
    return (en if lang == "en" else zh).get(ledger_type, ledger_type or "-")


def _ledger_deltas(row: dict) -> tuple[Decimal, Decimal]:
    ledger_type = str(row.get("type") or "")
    amount = _decimal_value(row.get("amount"))
    if row.get("frozen_before") is not None and row.get("frozen_after") is not None:
        frozen_delta = _decimal_value(row.get("frozen_after")) - _decimal_value(row.get("frozen_before"))
    else:
        frozen_delta = Decimal("0")
    if row.get("balance_before") is not None and row.get("balance_after") is not None:
        available_delta = _decimal_value(row.get("balance_after")) - _decimal_value(row.get("balance_before"))
    else:
        available_delta = Decimal("0")
    if frozen_delta != 0 or available_delta != 0:
        return frozen_delta, available_delta
    absolute = abs(amount)
    if ledger_type in {"bet_freeze", "withdraw_freeze", "admin_freeze"}:
        return absolute, -absolute
    if ledger_type == "bet_win_payout_frozen":
        return absolute, Decimal("0")
    if ledger_type in {"payout_unlock", "payout_unfreeze", "bet_win_payout_unlock", "frozen_release", "withdraw_reject_refund", "admin_unfreeze"}:
        return -absolute, absolute
    if ledger_type == "withdraw_paid":
        return -absolute, Decimal("0")
    return Decimal("0"), amount


def _format_ledger(rows: list[dict], lang: str = "zh") -> str:
    if not rows:
        return "No ledger records." if lang == "en" else "暂无账变记录。"
    if lang == "en":
        lines = ["Ledger", ""]
        labels = ("Reason", "Change", "Frozen Change", "Available Change")
    else:
        lines = ["账变记录", ""]
        labels = ("原因", "变动金额", "冻结资金变化", "可用余额变化")
    for row in rows:
        frozen_delta, available_delta = _ledger_deltas(row)
        lines.extend(
            [
                f"{labels[0]}：{_ledger_reason_label(str(row.get('type') or ''), lang)}",
                f"{labels[1]}：{_signed_money(row.get('amount'))}",
                f"{labels[2]}：{_signed_money(frozen_delta)}",
                f"{labels[3]}：{_signed_money(available_delta)}",
                "",
            ]
        )
    return "\n".join(lines).rstrip()
